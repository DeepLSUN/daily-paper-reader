---
title: "AIM: A Privacy-Aware Interoperable Memory Framework for Multi-Agent Multi-User LLM Systems"
title_zh: AIM：一种面向多智能体多用户LLM系统的隐私感知互操作记忆框架
authors: "Zachary Johnson, Nigel Boachie Kumankumah, Somya Chatterjee, Tejas Sathyamurthi, Min Chen, Xinyi Alice Li, Xiao Wang, Emily Morgan Gelchie, Jessica Lin, Sadid A. Hasan, Sulaiman Vesal"
date: 2026-09-11
pdf: "https://arxiv.org/pdf/2609.12320v1"
tags: ["query:agent"]
score: 7.0
evidence: 面向多智能体多用户LLM系统的记忆框架
tldr: "传统LLM局限于单用户会话，难以跨用户共享知识并追踪偏好演化；现有多智能体记忆多限于单用户，难兼顾隐私与共享。本文提出AIM，面向多智能体多用户LLM系统，动态划分私有与公共记忆，并用索引级访问控制保护私有数据。作者发布MUMBench多用户记忆基准，覆盖四领域及检索、创建、更新、删除操作。三次运行中AIM达96.0%可见性分类准确率、58.8%严格操作准确率与70.5%状态感知操作准确率，为多用户记忆管理提供统一方案与评测。"
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2609.12320v1/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1383, \"height\": 836}]"
tables_json: "[{\"url\": \"assets/tables/arxiv/2609.12320v1/table-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1456, \"height\": 765}, {\"url\": \"assets/tables/arxiv/2609.12320v1/table-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1508, \"height\": 1237}, {\"url\": \"assets/tables/arxiv/2609.12320v1/table-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 1454, \"height\": 185}, {\"url\": \"assets/tables/arxiv/2609.12320v1/table-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 1441, \"height\": 211}, {\"url\": \"assets/tables/arxiv/2609.12320v1/table-005.webp\", \"caption\": \"\", \"page\": 0, \"index\": 5, \"width\": 1640, \"height\": 233}]"
motivation: 传统LLM受限于单用户会话，无法跨用户共享知识或追踪偏好演化；现有多智能体记忆系统多局限于单用户，难以平衡隐私保护与共享知识。
method: 提出AIM隐私感知可互操作记忆框架，动态划分私有与公共信息，以索引级访问控制确保私有记忆仅所有者可检索，并发布MUMBench多用户基准。
result: "三次独立运行中，AIM取得96.0%可见性分类准确率、58.8%严格操作准确率和70.5%状态感知操作准确率。"
conclusion: AIM为多智能体多用户LLM系统提供兼顾隐私与共享的记忆管理方案，MUMBench则建立多用户记忆操作评测基准，推动该方向标准化评估。
---

## 摘要
传统大语言模型（LLM）的作用范围局限于单个用户会话，其知识被限制在单次对话中，无法学习随时间演变的用户偏好。现有的智能体记忆系统虽缓解了这一局限，但通常仅在单用户层面运行，限制了可在用户间共享以改进下游响应的公共知识。我们提出AIM（Agentic Interoperable Memory，智能体互操作记忆），一个统一的、隐私感知的记忆框架，使多智能体、多用户LLM系统能够持久化管理私有与共享记忆。AIM动态地将信息分类为私有（仅限某一用户访问，其他用户不可访问）或公共（所有用户均可访问）。它实施索引级访问控制，使私有记忆仅能由其所有者检索，从而在保护敏感数据的同时，允许有益的共享知识提升协调性与一致性。我们还提出MUMBench（Multi-User Memory Benchmark，多用户记忆基准），这是一个多用户交互数据集，涵盖四个领域中包含私有与可共享信息。据我们所知，MUMBench是首个旨在评估多用户环境中多种记忆操作（包括检索、创建、更新和删除）的公开数据集。在MUMBench上的三次独立运行中，AIM实现了96.0%的可见性分类准确率、58.8%的严格操作准确率和70.5%的状态感知操作准确率。

## Abstract
Traditional large language models (LLMs) are scoped to individual user sessions, limiting their knowledge to a single conversation and preventing them from learning user preferences that evolve over time. Existing agentic memory systems address this limitation but generally operate at the individual-user level, restricting the public knowledge that could be shared across users to improve downstream responses. We introduce AIM (Agentic Interoperable Memory), a unified, privacy-aware memory framework that enables multi-agent, multi-user LLM systems to persistently manage private and shared memory. AIM dynamically classifies information as private, scoped to one user and inaccessible to others, or public, accessible to all users. It enforces index-level access controls so that private memories are retrievable only by their owner, protecting sensitive data while allowing beneficial shared knowledge to improve coordination and consistency. We also introduce MUMBench (Multi-User Memory Benchmark), a dataset of multi-user interactions containing private and shareable information across four domains. To our knowledge, MUMBench is the first public dataset designed to evaluate multiple memory operations, including retrieval, creation, update, and deletion, in a multi-user environment. Across three independent runs on MUMBench, AIM achieves 96.0% visibility classification accuracy, 58.8% strict operation accuracy, and 70.5% state-aware operation accuracy.
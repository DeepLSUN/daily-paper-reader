...
tldr: 大语言模型在问答中可能产生幻觉，需要可靠的不确定性估计来选择性回答。现有不确定性分数缺乏统计保证，无法控制接受答案的错误率。本文提出CIC框架，基于置信区间校准将任意不确定性分数转化为风险控制规则，在多个模型和基准上实现有效风险控制并保持高回答率。该工作为可靠性敏感的QA部署提供了统计基础。
source: arxiv
...
motivation: 现有LLM不确定性分数缺乏统计保证，无法控制接受答案的错误率。
method: 提出CIC框架，通过校准集构建错误率的置信上界，选择满足风险水平的最大阈值。
result: 在七个LLM和多个基准上，CIC一致实现有效风险控制，同时保持高回答效率。
conclusion: CIC为LLM在可靠性敏感的QA中提供统计可靠的选择性回答机制。
---

## 摘要
大语言模型越来越多地部署在问答系统中，然而它们可能会在没有可靠置信度估计的情况下生成幻觉或未对齐的响应。不确定性量化提供了选择性回答的自然基础，即系统仅在其预测被认为可靠时才回答，否则弃权。然而，现有的大语言模型不确定性分数通常是启发式的：基于此类分数选择的阈值本身并不能为被接受答案中的错误率提供统计保证。我们提出CIC，一种基于置信区间的校准框架，将任意不确定性分数转化为风险控制的选择性回答规则。给定一个保留的校准集，CIC使用特定应用的对齐标准评估每个生成的响应，并将其与不确定性分数和二元错误标签相关联。对于每个候选不确定性阈值，CIC估计接受条件下的错误率，并使用Hoeffding风格或Clopper-Pearson置信区间构造高概率上置信界。然后，它选择上界低于用户指定风险水平α的最大阈值，从而在有限样本可靠性约束下最大化回答率。在可交换性条件下，CIC以至少1-δ的概率保证，如果选择的阈值非空，则控制被接受答案中的错误率在α水平。我们在七个大语言模型和多个不确定性估计器上对封闭式与开放式问答基准评估了CIC。实验结果表明，CIC在保持强大回答效率的同时始终实现有效的风险控制，为在可靠性敏感的问答工作流中部署大语言模型提供了一种实用且统计上合理的方法。

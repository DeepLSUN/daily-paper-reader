...
tldr: 现有LLM数据代理在异构企业环境中泛化能力差，需要高质量轨迹数据增强。TOFFEE系统通过蒙特卡洛树搜索结合自适应模型选择和跨任务前缀复用，从给定数据环境自动合成复杂分析轨迹。实验表明，其生成的轨迹能有效支持数据代理微调与上下文学习演示，覆盖多种异构环境。该系统提供了一个完整的框架，包括任务池构建、轨迹探索器和学习成本模型。
source: arxiv
...
motivation: 数据代理难以泛化到未知环境和异构工作流，需合成高质量轨迹以支持微调和上下文学习。
method: 采用蒙特卡洛树搜索结合自适应模型选择与跨任务前缀复用，系统化生成数据代理轨迹。
result: TOFFEE能高效生成可扩展的轨迹数据，适用于复杂分析任务和异构环境。
conclusion: TOFFEE为数据代理的适应性和推理提供了有效的轨迹合成方案，具有实用演示价值。
---

## 摘要
由大语言模型驱动的数据代理在数据驱动决策中发挥着越来越重要的作用。然而，现有的数据代理难以泛化到未见过的数据环境和分析工作流中，尤其是在异构的企业环境中。这催生了对合成高质量数据代理轨迹的日益增长的需求，这些轨迹能够捕捉给定数据环境的复杂分析工作流。此类轨迹支持两个关键的下游用途：它们可以作为监督微调（SFT）数据，使数据代理模型适应目标领域；也可以作为上下文学习（ICL）示例，指导通用大语言模型在陌生数据环境中进行推理。因此，我们引入了TOFFEE，一个通过蒙特卡洛树搜索（MCTS）结合自适应模型选择和跨任务前缀复用，从给定数据环境中合成高质量数据代理轨迹的系统。我们展示了TOFFEE能够有效地为跨异构环境的复杂分析任务生成可扩展的轨迹数据。在此演示中，我们介绍了TOFFEE的系统框架，包括其任务池构建、轨迹探索器和学习成本模型。我们还介绍了TOFFEE的网络界面及其工作流，并演示了两个端到端场景：用于数据代理微调的轨迹合成，以及演示增强的数据代理推理。
